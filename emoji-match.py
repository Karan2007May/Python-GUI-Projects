#Imports--------------------------------------
import os
from random import shuffle, randint
from guizero import App, Box, Picture, PushButton, Text

#Variables-------------------------------------

#set the path to the emoji folder on our computer
emojis_dir = "C:\\Users\\jivik\\OneDrive\\Desktop\\Code\\Coding\\Python\\GUIzero\\emojis"
emojis = [os.path.join(emojis_dir, f) for f in os.listdir(emojis_dir)]
shuffle(emojis)


#Functions------------------------------------

#Assigns emojis to pictures and buttons in the app
#Then makes sure the emojis match in both
def setup_round():
    for picture in pictures:
        picture.image = emojis.pop()

    for button in buttons:
        button.image = emojis.pop()
        button.update_command(match_emoji, args = [False])

    matched_emoji = emojis.pop()

    random_picture = randint(0,8)
    pictures[random_picture].image = matched_emoji

    random_button = randint(0,8)
    buttons[random_button].image = matched_emoji
    buttons[random_button].update_command(match_emoji, args=[True])

#Check if matched, announce result, setup next round
def match_emoji(matched):
    if matched:
        result.value = "Correct"
        score.value = int(score.value) + 1
    else:
        result.value = "Incorrect"
        score.value = int(score.value) - 1

    setup_round()

#Reduce timer by some seconds
def reduce_time():
    timer.value = int(timer.value) - 1
    #is it game over?
    if int(timer.value) < 0:
        result.value = "Game over! Score = " + score.value
        #hide the game
        game_box.hide()
        reset_button.show()

def reset():
    game_box.show()
    score.value = "0"
    timer.value = "30"
    result.value = ""
    reset_button.hide()

#App-------------------------------------------
app = App("emoji match")

#Status_box container for score and timer
game_box = Box(app, align = "top")

#Score and Timer
top_box = Box(game_box, align = "top", width = "fill")
top_box.bg = "#fdfdd0"
Text(top_box, align = "left", text = "Score:")
score = Text(top_box, text = "0", align="left")
timer = Text(top_box, text = '30', align="right")
Text(top_box, text="Time:", align = "right")

#emojis containers
pictures_box = Box(game_box, layout = 'grid', border = 9, align = "left")
buttons_box = Box(game_box, layout = "grid", border = 6, align = "right")

pictures = []
buttons = []
#Adding nine random emojis to pictures list and 
# buttons to buttons list
for x in range(0,3):
    for y in range(0,3):

        picture = Picture(pictures_box, grid = [x,y])
        pictures.append(picture)

        button = PushButton(buttons_box, grid = [x,y])
        buttons.append(button)

result = Text(app, bold = True, underline = True, size = 30)

reset_button = PushButton( app, command = reset,  text = "Play Another Round", visible = False)
reset_button.focus()
reset_button.bg = "#0000ff"
reset_button.text_color = "white"
reset_button.text_bold = True

setup_round()

app.repeat(1000, reduce_time)

app.display()